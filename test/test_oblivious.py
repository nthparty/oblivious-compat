...
class Test_oblivious_native(TestCase):
    ...
    def test_base(self, bits='080874618c0878927620101043a31002e840818101204000401210101261c120'):
        def fun(bs):
            return native.base(bs) if native.scalar(bs) else bytes([0])
        return check_or_generate_operation(self, fun, [32], bits)
    ...
    def test_mul(self, bits='28c5004000000100850000002102088891100000000081080810004280080004'):
        def fun(bs):
            ...
            return\
                native.mul(bs1, native.base(bs2))\
                if native.scalar(bs1) and native.scalar(bs2) else\
                bytes([0])
    ...
    def test_add(self, bits='0844000040000002818040008400000031080028000081080801000204081800'):
        def fun(bs):
            (bs1, bs2) = parts(bs, length=32)
            return\
                native.add(native.base(bs1), native.base(bs2))\
                if native.scalar(bs1) and native.scalar(bs2) else\
                bytes([0])
        return check_or_generate_operation(self, fun, [32, 32], bits)
    ...
    def test_sub(self, bits='002400041000800280800000a5024408111000800000810000008040a0081040'):
        def fun(bs):
            (bs1, bs2) = parts(bs, length=32)
            return\
                native.sub(native.base(bs1), native.base(bs2))\
                if native.scalar(bs1) and native.scalar(bs2) else\
                bytes([0])
        return check_or_generate_operation(self, fun, [32, 32], bits)


class Test_oblivious_sodium(TestCase):
    def test_scalar(self, bits='4df8fe738c097afa7f255b10c3ab118eeb73e38935605042ccb7581c73f1e5e9'):
        def fun(bs):
            return bitlist([1 if sodium.scalar(bs) else 0])
        return check_or_generate_operation(self, fun, [32], bits)

    def test_base(self, bits='080874618c0878927620101043a31002e840818101204000401210101261c120'):
        def fun(bs):
            return sodium.base(bs) if sodium.scalar(bs) else bytes([0])
        return check_or_generate_operation(self, fun, [32], bits)
    ...
    def test_mul(self, bits='28c5004000000100850000002102088891100000000081080810004280080004'):
        def fun(bs):
            (bs1, bs2) = parts(bs, length=32)
            return\
                sodium.mul(bs1, sodium.base(bs2))\
                if sodium.scalar(bs1) and sodium.scalar(bs2) else\
                bytes([0])
        return check_or_generate_operation(self, fun, [32, 32], bits)
    ...
    def test_add(self, bits='0844000040000002818040008400000031080028000081080801000204081800'):
        def fun(bs):
            (bs1, bs2) = parts(bs, length=32)
            return\
                sodium.add(sodium.base(bs1), sodium.base(bs2))\
                if sodium.scalar(bs1) and sodium.scalar(bs2) else\
                bytes([0])
        return check_or_generate_operation(self, fun, [32, 32], bits)
    ...
    def test_sub(self, bits='002400041000800280800000a5024408111000800000810000008040a0081040'):
        def fun(bs):
            (bs1, bs2) = parts(bs, length=32)
            return\
                sodium.sub(sodium.base(bs1), sodium.base(bs2))\
                if sodium.scalar(bs1) and sodium.scalar(bs2) else\
                bytes([0])
        return check_or_generate_operation(self, fun, [32, 32], bits)
...
if __name__ == "__main__":
    ...
    test_oblivious = Test_oblivious_native()
